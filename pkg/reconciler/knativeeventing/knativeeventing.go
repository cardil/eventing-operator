...
func (r *Reconciler) updateStatus(desired *eventingv1alpha1.KnativeEventing) (*eventingv1alpha1.KnativeEventing, error) {
	ke, err := r.KnativeEventingClientSet.EventingV1alpha1().KnativeEventings(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
	...
	return r.KnativeEventingClientSet.EventingV1alpha1().KnativeEventings(desired.Namespace).UpdateStatus(existing)
}
